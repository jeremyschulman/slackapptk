"""
The purpose of this module is to provide a CLI-like experience that would be
similiar if the software was a command-line tool built with Python standard
argparse.  The primary difference here is that the Slack workspace & surfaces
need to be accounted for; primarily the maximum number of characters and the
manner in which the help is reported back to the User; i.e. as a Slack message
rather than writing to sys.stdout.

With that said, the code below is could be vastly improved if I had a more
thorough understanding of the intracies of customizing the argparse
componentry.  The work below is a work in progress; and further refinements
should be expected.

TODO: Create better customized ArgumentParser and HelpFormatter classes
"""
